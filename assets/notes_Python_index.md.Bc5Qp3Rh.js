import{_ as n,c as e,o as t,a9 as r}from"./chunks/framework.BXUr4Z-K.js";const m=JSON.parse('{"title":"必知必会基础","description":"","frontmatter":{},"headers":[],"relativePath":"notes/Python/index.md","filePath":"notes/Python/index.md","lastUpdated":1737611781000}'),o={name:"notes/Python/index.md"},p=r(`<h1 id="必知必会基础" tabindex="-1">必知必会基础 <a class="header-anchor" href="#必知必会基础" aria-label="Permalink to &quot;必知必会基础&quot;">​</a></h1><p>1：为什么学习Python？</p><pre><code>因为他强大，上到抓外星人，下到喂小猫咪都能做。
...
</code></pre><p>4：Python解释器种类以及特点？</p><pre><code>CPython 从Python官网下载并且安装完后Python相应的解释器为CPython解释器。
...
</code></pre><p>关于这一点我在以前的文章特意说到过这个<a href="http://www.langzi.fun/Python%E8%A7%A3%E9%87%8A%E5%99%A8.html" target="_blank" rel="noreferrer">地址</a></p><p>5：位和字节的关系？</p><pre><code>位：&quot;位(bit)&quot;是电子计算机中最小的数据单位。每一位的状态只能是0或1。8个二进制位构成1个&quot;字节(Byte)&quot;，它是存储空间的基本计量单位。1个字节可以储存1个英文字母或者半个汉字，换句话说，1个汉字占据2个字节的存储空间。

字：&quot;字&quot;由若干个字节构成，字的位数叫做字长，不同档次的机器有不同的字长。例如一台8位机，它的1个字就等于1个字节，字长为8位。如果是一台16位机，那么，它的1个字就由2个字节构成，字长为16位。字是计算机进行数据处理和运算的单位。
</code></pre><p>6：b、B、KB、MB、GB 的关系？</p><pre><code>计算机存储信息的大小，最基本的单位是字节，一个汉字由两个字节组成，字母和数字由一个字节组成。 容量的单位从小到大依次是：字节（B）、KB、MB、GB、TB。它们之间的关系是
...
1KB=1024B
</code></pre><p>7：请至少列举5个PEP8规范（越多越好）。</p><pre><code>具体详情参见Python PEP8规范官网 https://legacy.python.org/dev/peps/pep-0008/
...
</code></pre><p>当然我这边还有一些笔记<a href="http://www.langzi.fun/PEP8%E8%A7%84%E5%88%99.html" target="_blank" rel="noreferrer">地址</a></p><p>8：isinstance作用？isinstance作用以及应用场景？</p><pre><code>isinstance 用来判断一个对象是否是一个已知的类型。
</code></pre><p>9：什么是面向对象的mro？</p><pre><code>解析类继承的查找顺序    MRO:Method Resolution Order,即方法解析顺序,是python中用于处理二义性问题的算法 。
</code></pre><p>我还以为是ORM。。。这下真的涨姿势了</p><p>10：Python的作用域？</p><pre><code>本地作用域（Local）→当前作用域被嵌入的本地作用域（Enclosing locals）→全局/模块作用域（Global）→内置作用域（Built-in）
</code></pre><p>python的作用域我以前特意写过<a href="http://www.langzi.fun/Python%20%E5%8F%98%E9%87%8F%E7%9A%84%E4%BD%9C%E7%94%A8%E5%9F%9F.html" target="_blank" rel="noreferrer">地址</a></p><p>11：装饰器的写法以及应用场景。</p><pre><code>我们回到日志的例子   并创建一个包裹函数，能让我们指定一个用于输出的日志文件。
...
</code></pre><p>装饰器说白了就是把闭包的东西改一下用到这里，使用系统自带的functools更加方便，具体原理我这边也有的哦<a href="http://www.langzi.fun/Python%20%E8%A3%85%E9%A5%B0%E5%99%A8.html" target="_blank" rel="noreferrer">链接</a></p><p>13：metaclass作用？以及应用场景？</p><pre><code>为了当创建类使能够自动地改变类。通常会为API做这样的事情，希望可以创建符合当前上下文的类。

用来指定指定某个类是由哪个类创建的
</code></pre><p>这个考到的python的面向对象编程中的class方法介绍，其实这些我用的很少，虽然当时只是做了笔记但是并没有融会贯通，我觉得使用函数比较方便(可能是因为的项目比较小，并且我的变量命名方法不够好)<a href="http://www.langzi.fun/Python%E9%9D%A2%E5%90%91%E5%AF%B9%E8%B1%A1%E7%BC%96%E7%A8%8B.html" target="_blank" rel="noreferrer">地址</a></p><p>14：什么是反射？以及应用场景？</p><pre><code>python有两个内置函数exec和eval，他们同样能够执行字符串。反射它的核心本质其实就是利用字符串的形式去对象（模块）中操作（查找/获取/删除/添加）成员，一种基于字符串的事件驱动！
</code></pre><p>15：1、2、3、4、5 能组成多少个互不相同且无重复的三位数？</p><pre><code>combinations_with_replacement 组合,有重复　　（有放回抽样组合）
...
60个
</code></pre><p>16：创建类有两种方式？</p><pre><code>特殊方式(基于元类的方式)

普通方式
</code></pre><p>17：列举面向对象中的特殊成员以及应用场景？</p><pre><code>__getitem__ 、 __setitem__ 、__delitem__ 用于索引操作，如字典。分别表示获取、设置、删除数据__str__ 如果一个类中定义了__str__方法，那么在打印对象时，默认输出该方法的返回值 __dict__ 查看类或对象中的所有成员 __call__ 对象后面加括号，触发执行__doc__  描述类的信息
</code></pre><p>18：静态方法和类方法区别？</p><pre><code>三种方法从不同层次上来对方法进行了描述：实例方法针对的是实例，类方法针对的是类，他们都可以继承和重新定义，而静态方法则不能继承，可以认为是全局函数。。

实例方法，类方法，静态方法都可以通过实例或者类调用，只不过实例方法通过类调用时需要传递实例的引用（python 3可以传递任意对象，其他版本会报错）。
</code></pre><p>19：如何判断是函数还是方法？</p><pre><code>看他的调用者是谁，如果是类，需要传入一个参数self的值，这时他就是一个函数，如果调用者是对象，不需要给self传入参数值，这时他是一个方法。
</code></pre><p>20：列举面向对象中带爽下划线的特殊方法？</p><pre><code>当创建一个新实例时调用__new__,初始化一个实例时用__init__。只有在__new__返回一个cls的实例时后面的__init__才能被调用。__new__方法会返回一个创建的实例,而__init__什么都不返回。
...
__new__
</code></pre><p>21：是否使用过functools中的函数？其作用是什么？</p><pre><code>用于装饰器中，functools.wraps()，主要是用来修复装饰器。实际上固定了int()函数的关键字参数base  如 int2 = functools.partial(int, base=2)。
...
</code></pre><p>22：面向对象中super的作用？</p><pre><code>在super机制里可以保证公共父类仅被执行一次，至于执行的顺序，是按照MRO（Method Resolution Order），方法解析顺序进行的。MRO 就是类的方法解析顺序表, 其实也就是继承父类方法时的顺序表。

super 是用来解决多重继承问题的，直接用类名调用父类方法在使用单继承的时候没问题，但是如果使用多继承，会涉及到查找顺序（MRO）、重复调用（钻石继承）等种种问题。

super() 函数是用于调用父类(超类)的一个方法。
</code></pre><p>23：面向对象深度优先和广度优先是什么？</p><pre><code>深度优先：不全部保留节点，占用空间小，有回溯操作（即有入栈/出栈操作），运行速度慢。

广度优先：保留全部节点，占用空间大；无回溯操作（既无入栈、出栈操作）、运行速度快。
</code></pre><p>24：Python面向对象中的继承有什么特点？</p><pre><code>在继承中基类的构造（__init__()方法）不会被自动调用，它需要在其派生类的构造中亲自专门调用。有别于C#。
...
</code></pre><p>25： 什么是重载？</p><pre><code>重载就是类里面相同方法名，不同形参的情况，可以是形参类型不同或者形参个数不同，或者形参顺序不同，但是不能使返回值类型不同。
</code></pre><p>26：什么是重写？</p><pre><code>重写很简单就是把子类从父亲类里继承下来的方法重新写一遍，这样，父类里相同的方法就被覆盖了，当然啦，你还是可以通过super.CaculSalary方法来调用父类的工资计算方法。
</code></pre><p>27：什么是多态？</p><pre><code>我们在有一些方法在父类已经定义好了，但是子类我们自己再用的时候，发现，其实，我们的虽然都是计算工资的，但是普通员工的工资计算方法跟经理的计算方法是不一样的，所以这个时候，我们就不能直接调用父类的这个计算工资的方法了。这个时候我们就需要用到面向对象的另一个特性，多态。我们要在子类里面把父类里面定义计算工资的方法在子类里面重新实现一遍。多态包含了重载和重写。
</code></pre><p>28：谈谈你对面向对象的理解？</p><pre><code>所谓的面向对象就是将我们的程序模块化，对象化，把具体事物的特性属性和通过这些属性来实现一些动作的具体方法放到一个类里面，这就是封装。封装是我们所说的面相对象编程的特征之一。除此之外还有继承和多态。
</code></pre><p>29：如何使用python删除一个文件？</p><pre><code>import shutil shutil.rmtree()。
...
</code></pre><p>30：如何生成一个随机数？</p><pre><code>random.uniform(a,b):返回指定范围内的浮点数。
...
</code></pre><p>31：os和sys模块的作用？</p><pre><code>os.chmod(&#39;pathfile&#39;,os.W_OK) 改变文件权限模式。
...
</code></pre><p>相关资料链接<a href="http://www.langzi.fun/Python%20os%20%E6%A8%A1%E5%9D%97.html" target="_blank" rel="noreferrer">地址os</a> <a href="http://www.langzi.fun/Python%20sys%20%E6%A8%A1%E5%9D%97.html" target="_blank" rel="noreferrer">地址sys</a></p><p>32：谈谈你对闭包的理解？</p><pre><code>一般情况下，在我们认知当中，如果一个函数结束，函数的内部所有东西都会释放掉，还给内存，局部变量都会消失。但是闭包是一种特殊情况，如果外函数在结束的时候发现有自己的临时变量将来会在内部函数中用到，就把这个临时变量绑定给了内部函数，然后自己再结束。

在一个外函数中定义了一个内函数，内函数里运用了外函数的临时变量，并且外函数的返回值是内函数的引用。这样就构成了一个闭包。
</code></pre><p>33：用Python实现一个二分查找的函数。</p><pre><code>def binary_search(data_set, data):
...
</code></pre><p>关于算法这边我忙不过来，但是以后肯定会深入学习的，目前我只会插入排序，选择排序和二分法<a href="http://www.langzi.fun/Python%E7%AE%97%E6%B3%95%E4%B9%8B%E6%8E%92%E5%BA%8F%E4%B8%8E%E4%BA%8C%E5%88%86%E6%B3%95.html" target="_blank" rel="noreferrer">地址</a></p><p>34:简述 生成器、迭代器、可迭代对象 以及应用场景？</p><pre><code>有__iter__和__next__方法的对象就是一个迭代器，但是迭代器不一定是生成器。
...
</code></pre><p>答案这边说的其实不准确具体看</p><pre><code>迭代器(Iterator)与可迭代对象(Iterable)的区别
...
一般由可迭代对象变迭代器对象
</code></pre><p><a href="http://www.langzi.fun/Python%20%E7%94%9F%E6%88%90%E5%99%A8%E4%B8%8E%E7%94%9F%E6%88%90%E5%8F%AF%E8%BF%AD%E4%BB%A3%E5%AF%B9%E8%B1%A1.html" target="_blank" rel="noreferrer">地址1</a> <a href="http://www.langzi.fun/Python%20%E7%94%9F%E6%88%90%E5%99%A8%E4%B8%8E%E7%94%9F%E6%88%90%E5%8F%AF%E8%BF%AD%E4%BB%A3%E5%AF%B9%E8%B1%A1.html" target="_blank" rel="noreferrer">地址2</a></p><p>35：常用字符串格式化哪几种？</p><pre><code>print(&#39;hello {first} and {second}&#39;.format(first=&#39;df&#39;,second=&#39;another df&#39;))

print(&#39;hello %(first)s and %(second)s&#39;%{&#39;first&#39;:&#39;df&#39; , &#39;second&#39;:&#39;another df&#39;})

print(&#39;hello %s and %s&#39;%(&#39;df&#39;,&#39;another df&#39;))
</code></pre><p>36：请用代码简答实现stack Queue。</p><pre><code>queue就是管道，分成三种模式。
定义
q=Queue.queue()
传入数据
q.put(1)
...
还可以获取里面的数据是否没空
if q.empty()
</code></pre><p>管道可以用来存储数据，作为中转，可以写出一个简单的生产者消费者模式<a href="http://www.langzi.fun/Python%20Queue%20%E6%A8%A1%E5%9D%97.html" target="_blank" rel="noreferrer">地址</a></p><p>37：logging模块的作用？以及应用场景？</p><pre><code>简单来讲就是，我们通过记录和分析日志可以了解一个系统或软件程序运行情况是否正常，也可以在应用程序出现故障时快速定位问题。比如，做运维的同学，在接收到报警或各种问题反馈后，进行问题排查时通常都会先去看各种日志，大部分问题都可以在日志中找到答案。再比如，做开发的同学，可以通过IDE控制台上输出的各种日志进行程序调试。对于运维老司机或者有经验的开发人员，可以快速的通过日志定位到问题的根源。可见，日志的重要性不可小觑通过log的分析，可以方便用户了解系统或软件、应用的运行情况；如果你的应用log足够丰富，也可以分析以往用户的操作行为、类型喜好、地域分布或其他更多信息；如果一个应用的log同时也分了多个级别，那么可以很轻易地分析得到该应用的健康状况，及时发现问题并快速定位、解决问题，补救损失。

软件程序运行故障分析与问题定位

程序调试
</code></pre><p>具体使用方法<a href="http://www.langzi.fun/Python%20logging%20%E6%A8%A1%E5%9D%97.html" target="_blank" rel="noreferrer">地址</a></p><p>38：日志级别的种类。</p><pre><code>CRITICAL 当发生严重错误，导致应用程序不能继续运行时记录的信息
...
</code></pre><p>39：如何在函数中设置一个全局变量 ?</p><pre><code>在函数的内部，通过global声明，使在函数内部中设置一个全局变量，这个全局变量可以在任意的函数中进行调用。
</code></pre><p>40：一行代码实现删除列表中重复的值 ?</p><pre><code>print(sorted(set(list(列表)),key=list(列表).index)) # sorted output
</code></pre><p>41：如何用一行代码生成[1,4,9,16,25,36,49,64,81,100] ?</p><pre><code>a = [pow(i,2) for i in range(1,11)]
</code></pre><p>42：比较a = [1,2,3] 和 b = [(1),(2),(3) ] 以及 b = [(1,),(2,),(3,) ] 的区别？</p><pre><code>print([1, 2, 3])               [1, 2, 3]
...
c 列表里面的每个元素都是元组。
</code></pre><p>43：如何实现[‘1’,’2’,’3’]变成[1,2,3] ?</p><pre><code>第一种：a = [int(i) for i in a]
...
</code></pre><p>49：re的match和search区别？</p><pre><code>search：从字符的起始位置开始匹配，匹配到第一个符合的就不会去匹配了。

match：尝试从字符串起始位置开始匹配如果没有就会返回None。

使用search的话语要用group来获取结果，group(0)就是获取匹配符加结果一起的，group(1)就是匹配到的结果
</code></pre><p>50：至少列举8个常用模块都有那些？</p><pre><code>re，os，random，json，time，numpy，opencv-python，pytz，pyMysql，pygame，pillow-	python，pip，Django
</code></pre><p>51：如何安装第三方模块？以及用过哪些第三方模块？</p><pre><code>在cmd终端可以使用pip install 安装第三方模块。
...
</code></pre><p>54：对给定的List L进行排序，对List、Dict进行排序，Python提供了两个方法。</p><pre><code>用List的成员函数sort进行排序，在本地进行排序，不返回副本。
...
</code></pre><p>55：列举常见的内置函数？</p><pre><code>数据集合：len，sorted，enumerate，all，any，zip，filter，map
...
</code></pre><p>56：求</p><pre><code>v = dict.fromkeys([&#39;k1&#39;, &#39;k2&#39;], [33])
...
{&#39;k1&#39;: 777, &#39;k2&#39;: [666]}
</code></pre><p>57：Python的可变类型和不可变类型？</p><pre><code>对于不可变类型，无论创建多少个不可变类型，只要值相同，都指向同个内存地址（若值不同，那么一定指向不同的内存地址）。
...
</code></pre><p>58：Python垃圾回收机制？</p><pre><code>分代收集：Python把内存根据对象存活时间划分为三代，对象创建之后，垃圾回收器会分配它们所属的代。每个对象都会被分配一个代，而被分配更年轻的代是被优先处理的，因此越晚创建的对象越容易被回收。连在一起，构成一个有向图，对象构成这个有向图的节点，而引用关系构成这个有向图的边）。
...
</code></pre><p>这个确实涉及到短板了，需要补充这方的知识点</p><p>59：简述Python的深浅拷贝以及应用场景？</p><pre><code>浅拷贝指仅仅拷贝数据集合的第一层数据，深拷贝指拷贝数据集合的所有层。所以对于只有一层的数据集合来说深浅拷贝的意义是一样的，比如字符串，数字，还有仅仅一层的字典、列表、元祖等。

深拷贝：copy.deepcopy	浅拷贝：copy.copy

导入模块：import copy
</code></pre><p>60：is和==的区别。</p><pre><code>is是对比地址，==是对比值。
</code></pre><p>61：*arg和**kwarg作用。</p><pre><code>接收参数。
...
*arg：元组或列表“出现”。
</code></pre><p>62：pass的作用？</p><pre><code>保证语义完整

保证格式完整

空语句 do nothing
</code></pre><p>63：lambda表达式格式以及应用场景？</p><pre><code>为了解决那些功能很简单的需求而设计的一句话函数。
</code></pre><p>64：字符串常用功能？</p><pre><code>长度 循环 键、值、键值对 删除 新增 索引 字典常用操作 包含 长度 切片 索引 元组常用操作 包含 循环 切片 长度  删除 追加 切片 索引 列表常用操作 切片 索引 长度 分割 移除空白
...
</code></pre><p>65：列举布尔值为False的常见值？</p><pre><code>False 0 [] () {} 字符串 布尔值 零 列表  元组 字典。
</code></pre><p>66：xreadlines和readlines的区别？</p><pre><code>二者使用时相同，但返回类型不同，xreadlines返回的是一个生成器，readlines返回的是list。
</code></pre><p>67：xrange和range的区别？</p><pre><code>Python 3.x已经去掉xrange，全部用range代替。
...
</code></pre><p>68：Python3和Python2中 int 和 long的区别？</p><pre><code>long整数类型被Python3废弃，统一使用int
</code></pre><p>69：用一行代码实现数值交换。</p><pre><code>b = 2   a = 1

a,b = b,a
</code></pre><p>70：列举 Python2和Python3的区别？</p><pre><code>在Python2中，print 被视为一个语句而不是一个函数，这是一个典型的容易弄混的地方，因为在 Python 中的许多操作都需要括号内的参数来执行。
</code></pre><p>71：三元运算规则以及应用场景？</p><pre><code>(expr1) ? (expr2) : (expr3);
...
a = 7 if x&gt;0 else 5
print(a)
</code></pre><p>72：字节码和机器码的区别？</p><pre><code>机器码存在着多至100000种机器语言的指令。这意味着不能把这些种类全部列出来。尽管机器语言好像是很复杂的，然而它是有规律的。8086到Pentium的机器语言指令长度可以从1字节到13字节。机器语言是微处理器理解和使用的，用于控制它的操作二进制代码。用机器语言编写程序，编程人员要首先熟记所用计算机的全部指令代码和代码的涵义。手编程序时，程序员得自己处理每条指令和每一数据的存储分配和输入输出，还得记住编程过程中每步所使用的工作单元处在何种状态。这是一件十分繁琐的工作，编写程序花费的时间往往是实际运行时间的几十倍或几百倍。而且，编出的程序全是些0和1的指令代码。总结：机器码是电脑CPU直接读取运行的机器指令，运行速度最快机器码就是计算机可以直接执行，并且执行速度最快的代码。
...
</code></pre><p>73：ascii、unicode、utf-8、gbk 区别？</p><pre><code>UTF-8（8-bit Unicode Transformation Format）是一种针对Unicode的可变长度字符编码，又称万国码。由Ken Thompson于1992年创建。现在已经标准化为RFC 3629。UTF-8用1到6个字节编码UNICODE字符。用在网页上可以同一页面显示中文简体繁体及其它语言（如英文，日文，韩文）。
...
</code></pre><p>74：求出下列的结果。</p><pre><code>v6 = 0 or False and 1       False
...
</code></pre><p>76：python递归的最大层数？</p><pre><code>程序表明在3925-3929之间  一般是在998
</code></pre><p>77：请编写一个函数实现将IP地址转换成一个整数。</p><pre><code>def ipfunc(ip):
...
</code></pre><p>78：Python编程常用技巧？</p><pre><code>#使用yield节省内存开销
...
empty
</code></pre><p>80：json序列化时，默认遇到中文会转换成unicode，如果想要保留中文怎么办？如果需要保留date或者datetime日期格式的数据，该如何处理？</p><pre><code>第一种方案：定义处理类。
...
</code></pre><p>81：什么是断言？应用场景？</p><pre><code>Python的assert是用来检查一个条件，如果它为真，就不做任何事。如果它为假，则会抛出AssertError并且包含错误信息。例如：
...
不要用太多断言以至于让代码很晦涩。
</code></pre><p>82：有用过with statement吗？它的好处是什么？</p><pre><code>with语句的作用是通过某种方式简化异常处理，它是所谓的上下文管理器的一种。
...
</code></pre><p>83：使用代码实现查看列举目录下的所有文件。</p><pre><code>import os

# 可以通过相对路径    print(os.listdir(&#39;../文件名称&#39;))

#也可以通过绝对路径  得到的结果是一个列表的形式可以遍历出来   print(os.listdir(&#39;绝对路径&#39;))
</code></pre><p>84：简述 yield和yield from关键字。</p><pre><code>yield from iterable本质上等于for item in iterable: yield item的缩写。
...
</code></pre><p>85：提高python运行效率的方法。</p><pre><code>使用生成器，因为可以节约大量内存。
...
</code></pre><p>86：IOError AttributeError ImportError IndentationError IndexError KeyError SyntaxError NameError分别代表什么异常。</p><pre><code>IOError：输入输出异常。
...
IndentationError：语法错误，代码没有正确的对齐。
</code></pre><p>86： *args和**kwargs是什么意思？</p><pre><code>*args表示可变参数（variadic arguments），它允许你传入0个或任意个无名参数，这些参数在函数调用时自动组装为一个tuple； **kwargs表示关键字参数（keyword arguments），它允许你传入0个或任意个含参数名的参数，这些关键字参数在函数内部自动组装为一个dict。同时使用*args和**kwargs的时候，必须保证*args在**kwargs之前。
</code></pre><p>87: python里面如何拷贝一个对象？</p><pre><code>（1） 赋值（=），就是创建了对象的一个新的引用，修改其中任意一个变量都会影响到另一个；
...
注意：并不是所有的对象都可以拷贝
</code></pre><p>88: 简要描述python的垃圾回收机制</p><pre><code>python中的垃圾回收是以引用计数为主，标记-清除和分代收集为辅。
...
</code></pre><p>89: 什么是lambda函数？它有什么好处？</p><pre><code>lambda表达式，通常是在需要一个函数，但是又不想费神去命名一个函数的场合下使用，也就是指匿名函数。

Python允许你定义一种单行的小函数。定义lambda函数的形式如下（lambda参数：表达式）lambda函数默认返回表达式的值。你也可以将其赋值给一个变量。lambda函数可以接受任意个参数，包括可选参数，但是表达式只有一个。
</code></pre><p>90: python如何实现单例模式？</p><pre><code>单例模式是一种常用的软件设计模式。在它的核心结构中只包含一个被称为单例类的特殊类。通过单例模式可以保证系统中一个类只有一个单例而且该单例易于外界访问，从而方便对实例个数的控制并节约系统资源。如果希望在系统中某个类的对象只能存在一个，单例模式是最好的解决方案。
</code></pre><p>91: python自省</p><pre><code>自省就是面向对象的语言所写的程序在运行时，所能知道对象的类型，简单一句就是运行时能够获得对象的类型，比如type(),dir(),getattr(),hasattr(),isinstance().
</code></pre><p>举个例子：</p><pre><code>a = [1,2,3]
b = {&#39;a&#39;:1,&#39;b&#39;:2,&#39;c&#39;:3}
c = True
print type(a),type(b),type(c) # &lt;type &#39;list&#39;&gt; &lt;type &#39;dict&#39;&gt; &lt;type &#39;bool&#39;&gt;
print isinstance(a,list)  # True
</code></pre><p>92： 谈一谈python的装饰器</p><pre><code>装饰器本质上是一个python函数，它可以让其他函数在不作任何变动的情况下增加额外功能，装饰器的返回值也是一个函数对象。它经常用于有切面需求的场景。比如：插入日志、性能测试，事务处理、缓存、权限验证等。有了装饰器我们就可以抽离出大量的与函数功能无关的雷同代码进行重用。
